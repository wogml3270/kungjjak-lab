...
export default async function MyPage({
  ...
  const roomIds = [
    ...new Set((membershipResult.data ?? []).map(({ room_id }) => room_id).filter(Boolean)),
  ];
  const membershipByRoom = new Map(
    (membershipResult.data ?? []).map((membership) => [membership.room_id, membership.id]),
  );
  const [roomResult, reportResult, participantResult, responseResult, questionResult] =
    roomIds.length
      ? await Promise.all([
          supabase
            .from('rooms')
  ...
  if (participantResult.error)
    console.error('[mypage] participant name query failed', participantResult.error);
  ...
  const coOpHistories = (roomResult.data ?? []).map((room) => {
    ...
    const profiles = (profilesByRoom.get(room.id) ?? []).sort((first, second) =>
    ...
    const axisResults = axes.map((axis) => {
      const axisPairs = entries.filter(
        ([questionId]) => questionsById.get(questionId)?.dimension === axis.dimension,
      );
      ...
      const tendency = axisPairs.reduce(
        (sum, [questionId, values]) =>
          sum +
          (values[0] + values[1]) *
            (questionsById.get(questionId)?.positive_trait === axis.leftTrait ? 1 : -1),
        0,
      );
    ...
    return {
      ...
      profiles:
        profiles.length === 2
          ...
          : [
              ...
              { name: '상대방', avatarUrl: '/default-profile.svg', role: 'guest' },
            ],
      ...
      strongMatches: pairs.filter(([first, second]) => Math.abs(first) === 2 && first === second)
        .length,
      axisResults,
      gap: biggestGap.gap,
