...
export default async function CoOpResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!report) {
    const { data: room } = await supabase.from('rooms').select('id').eq('id', roomId).eq('status', 'completed').maybeSingle();
    if (!room) notFound();
  }
  const [{ data: participants, error: participantError }, { data: responses, error: responseError }] = await Promise.all([
  ...
  for (const response of responses ?? []) byQuestion.set(response.question_id, [...(byQuestion.get(response.question_id) ?? []), response.score_value]);
  ...
  const score = report ? Number(report.score) : pairs.length === 24 ? Math.round((1 - difference / 96) * 100) : 0;
  const summary = report?.summary ?? '24문항 쿵짝 실험 완료';

  return <main className="mx-auto flex min-h-screen max-w-md items-center px-5 py-10"><section className="w-full rounded-3xl border-3 border-black bg-brand-mint p-6 text-center shadow-neo-lg"><p className="text-xs font-black tracking-widest">CO-OP RESULT</p><span aria-hidden className="mt-4 block text-6xl">💞</span><h1 className="mt-4 text-3xl font-black">우리의 쿵짝 스코어</h1><p className="mt-2 text-sm font-black">{names.length === 2 ? names.join(' × ') : '나 × 상대방'}</p><p className="mt-3 text-7xl font-black">{Math.round(score)}<span className="text-3xl">%</span></p><p className="mt-4 font-bold">{summary}</p><Link className="neo-button mt-7 flex items-center justify-center bg-brand-yellow" href="/mypage?tab=co-op">← 2인 기록 목록으로</Link></section></main>;
}
