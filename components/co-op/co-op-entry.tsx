  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user && !user.is_anonymous) {
        const nickname = String(user.user_metadata.service_nickname ?? user.user_metadata.full_name ?? user.user_metadata.name ?? '쿵짝 연구원').trim().slice(0, 10);
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md items-center px-5 py-10">
      ...
      >
        ...
        {!authResolved ? <p className="mt-6 rounded-xl border-2 border-black bg-white p-3 text-center text-sm font-black">계정 정보를 확인하고 있어요…</p> : signedIn ? <div className="mt-6 rounded-xl border-2 border-black bg-brand-mint p-3 text-sm font-bold"><span className="font-black">{displayName}</span> 이름으로 참여해요.</div> : <><label className="mt-6 block text-sm font-black" htmlFor="display-name">임시로 사용할 이름</label><p className="mt-1 text-xs font-bold">비로그인 참여용 이름이에요. 로그인하면 설정한 닉네임이 자동 적용돼요.</p><input className="mt-2 w-full rounded-xl border-3 border-black bg-white px-4 py-3 font-bold shadow-neo" id="display-name" maxLength={10} onChange={(event) => setDisplayName(event.target.value)} placeholder="예: 재희 (최대 10자)" value={displayName} /></>}

        <button className="neo-button mt-4 w-full bg-brand-yellow disabled:cursor-wait disabled:opacity-60" disabled={isLoading || !authResolved} onClick={createRoom} type="button">
        ...
        <form className="flex gap-2" onSubmit={joinRoom}>
          ...
          <input
            ...
            value={roomCode}
          />
          <button className="neo-button bg-brand-mint disabled:opacity-40" disabled={!authResolved} type="submit">입장</button>
        </form>
