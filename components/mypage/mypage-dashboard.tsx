...
function SoloResultDrawer({ onClose, onDelete, result }: { onClose: () => void; onDelete: (item: SoloHistory) => Promise<void>; result: SoloHistory | null }) {
  useEffect(() => {
    if (!result) return;
...
function ResultDrawer({ onClose, onDelete, result }: { onClose: () => void; onDelete: (item: CoOpHistory) => Promise<void>; result: CoOpHistory | null }) {
  useEffect(() => {
    if (!result) return;
    const previousOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
...
function DangerZone({ onDelete }: { onDelete: () => void }) { return <div className="mb-3 mt-7 border-t-2 border-dashed border-neutral-400 pt-5"><button className="w-full rounded-xl border-2 border-black bg-brand-pink px-4 py-3 text-sm font-black shadow-[2px_2px_0_#000] transition-transform hover:-translate-y-0.5 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none" onClick={onDelete} type="button">이 기록 삭제하기</button><p className="mt-2 text-center text-[10px] font-bold text-neutral-500">삭제한 기록은 복구할 수 없어요.</p></div>; }
