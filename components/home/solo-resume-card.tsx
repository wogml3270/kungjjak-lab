  ...
  useEffect(() => {
    const syncProgress = async () => {
      ...
      try {
        ...
        const { data } = await createClient().from('questions').select('title').eq('id', questionId).single();
    ...
    void syncProgress();
    const handleProgressChange = () => { void syncProgress(); };
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 pr-8">
        <div>
          ...
          <h2 className="mt-1 line-clamp-2 text-lg font-black leading-7">“{questionTitle || '질문을 불러오는 중…'}”</h2>
        </div>
